...
const Home = () => {
    ...
    const { data: lancamentos } = useQuery({ queryKey: ['destaques'], queryFn: () => obterLivrosDestaque('lancamentos') })
    const { data: maisVendidos } = useQuery({ queryKey: ['maisVendidos'], queryFn: () => obterLivrosDestaque('mais-vendidos') })
